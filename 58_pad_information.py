...
urls = ['http://sz.58.com/pbdn/0/pn{}/'.format(str(i)) for i in range(1, 20)]

def get_target_urls(url):
    urls = list()
    web_response_data = requests.get(url)
    soup = BeautifulSoup(web_response_data.text, 'lxml')

    #pad_informations = soup.select('table.tbimg > tbody > tr.jztr')
    pad_informations = soup.select('tr.jztr')
...
def extract_infos(urls):
    datas = list()
    for url in urls:
        web_response_data = requests.get(url['url'])
        soup = BeautifulSoup(web_response_data.text, 'lxml')
        ...
        try:
            region = soup.select('span.c_25d')[0].get_text().strip()
        except Exception:
            region = None
        headers = {
...
urls = get_target_urls(url)



infos = extract_infos(urls)
